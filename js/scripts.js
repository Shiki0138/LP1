...
window.addEventListener('load', function() {
  ...
  // モバイルでもAOSを有効にしたい場合は disable: true を外すか false にする
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // モバイル無効を完全にオフにしたいなら disable: false に変更
  AOS.init({
      duration: 800,
      easing: 'ease-in-out',
      once: true,
  ...
  // ギャラリーアイテムのクリックイベント（モーダル表示など）
  const galleryItems = document.querySelectorAll('.gallery-item');
  
  galleryItems.forEach(item => {
      item.addEventListener('click', function() {
          // ここにモーダル表示のコードを追加可能
          console.log('ギャラリーアイテムがクリックされました');
      });
  });
  ...
  // 画像の遅延読み込み (IntersectionObserver + data-src の場合)
      ...
      const imageObserver = new IntersectionObserver((entries, observer) => {
          entries.forEach(entry => {
              if (entry.isIntersecting) {
                  ...
                  // ★ 実際に利用するなら data-src を指定して src を差し替える
